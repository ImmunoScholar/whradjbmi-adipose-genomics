...
To reproduce from scratch: download the GWAS file, GSE129363 supplementary
files, and the liftOver chain to `data/raw/` per the paths in
`config/config.yml`, then run `scripts/01_*.R` through `scripts/09_*.R` in
order.
## Results
...
comparison), and `results/figures/` (Manhattan plot, colocalization
diagnostic, UMAP, candidate-gene dot plots, integrated evidence figure).
## Limitations
